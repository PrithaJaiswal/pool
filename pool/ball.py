...
class BallSprite(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        if np.hypot(*self.ball.velocity) != 0:
            ...
            perpendicular_velocity = -np.cross(
                self.ball.velocity,
                [0, 0, 1]
            )
    ...
    def update_sprite(self):
        ...
        if self.label_offset[2] > 0:
            ...
            if self.label_offset[0] != 0:
                ...
                label = pygame.transform.scale(
                    label,
                    (
                        int(config.ball_radius * dist_from_centre),
                        config.ball_radius
                    )
                )
        ...
        new_sprite.blit(
            label,
            self.label_offset[:2] +
            (sprite_dimension - label.get_size()) / 2
        )
        ...
        self._apply_circular_mask(new_sprite, colorkey)
    ...
    def _apply_circular_mask(self, sprite, colorkey):
        grid_2d = np.mgrid[
        ...
        for xy in itertools.product(
            range(config.ball_radius * 2 + 1),
            repeat=2
        ):
            if is_outside[xy]:
                sprite.set_at(xy, colorkey)
    ...
    def is_active(self, game_state, behind_separation_line=False):
        ...
        while events["clicked"]:
            ...
            # checks if the user isn't trying to place the ball out of the
            # table or inside another ball
            if (
                np.all(
                    np.less(
                        config.table_margin +
