...
class Cue(pygame.sprite.Sprite):
    ...
    def _create_clear_surface(self):
        surface = pygame.Surface(2 * self.sprite_size)
        surface.fill((200, 200, 200))
        surface.set_colorkey((200, 200, 200))
        return surface

    def clear_canvas(self):
        # create empty surface as a placeholder for the cue
        self.image = self._create_clear_surface()
    ...
    def update(self, *args):
        if self.visible:
            ...
            rectangle_points_from_circle = (
                rectangle_points + self.displacement * sin_cos
            )
            ...
            self.points_on_screen = (
                rectangle_points_from_circle + self.target_ball.ball.pos
            )
    ...
    def draw_lines(self, game_state, target_ball, angle, color):
        ...
        while (
            config.resolution[1] > cur_pos[1] > 0
            and config.resolution[0] > cur_pos[0] > 0
        ):
            cur_pos += config.aiming_line_length * diff * 2

            pygame.draw.line(
                game_state.canvas.surface,
                color,
                cur_pos,
    ...
    def is_clicked(self, events):
        return (
            events["clicked"]
            and self.is_point_in_cue(events["mouse_pos"])
        )
    ...
    def cue_is_active(self, game_state, events):
        ...
        if self.displacement > (
            config.ball_radius + config.cue_safe_displacement
        ):
            self.ball_hit()
    ...
    def ball_hit(self):
        new_velocity = -(
            self.displacement
            - config.ball_radius
            - config.cue_safe_displacement
        ) * config.cue_hit_power * np.array([
            math.sin(self.angle),
            math.cos(self.angle)
        ])
    ...
    def update_cue(self, game_state, initial_mouse_dist, events):
        ...
        displacement_from_ball_to_mouse = (
            self.target_ball.ball.pos - current_mouse_pos
        )
