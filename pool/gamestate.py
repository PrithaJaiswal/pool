...
class GameState:
    ...
    def create_white_ball(self):
        self.white_ball = ball.BallSprite(config.WHITE_BALL_NUMBER)
        ball_pos = config.white_ball_initial_pos

        while check_if_ball_touches_balls(
            ball_pos,
            config.WHITE_BALL_NUMBER,
            self.balls
        ):
            ball_pos = [
                random.randint(
    ...
    def set_pool_balls(self):
        ...
        initial_place = (
            config.ball_starting_place_ratio
            * config.resolution
        )
        ...
        ball_placement_sequence = list(
            range(1, config.total_ball_num)
        )
        ...
        for i in ball_placement_sequence:
            ...
            ball_iteration.move_to(
                initial_place + coord_shift * counter
            )
    ...
    def generate_table(self):
        ...
        all_hole_positions = np.vstack(
            (
                all_hole_positions[:3],
                np.flipud(all_hole_positions[3:])
            )
        )
        ...
        for hole_pos in all_hole_positions:
            ...
            #
            # they are generated using 4x2 offset matrices
            # (4 2d points around the hole)
            # with the first point in the matrix being the starting point
            # and the last point being the ending point
            #
            # the matrices must be flipped using numpy.flipud()
            # after reflecting them using 2x1 reflection matrices, otherwise
            # starting and ending points would be reversed

            if hole_pos[0][1] == 2:
                # hole_pos[0,1]=2 means x coordinate ID is 2
                # which means this hole is in the middle
            ...
            table_side_points = np.append(
                table_side_points,
                [hole_pos[0][0], hole_pos[1][0]] + offset,
                axis=0
            )

        # deletes the 1st point in array (leftover from np.empty)
        table_side_points = np.delete(
            table_side_points,
            0,
            0
        )
    ...
    def game_over(self, p1_won):
        font = config.get_default_font(
            config.game_over_label_font_size
        )
    ...
    def check_potted(self):
        # if white ball is potted, it will be created again
        ...
        if config.WHITE_BALL_NUMBER in self.potted:
            ...
            self.potted.remove(config.WHITE_BALL_NUMBER)
            self.turn_over(True)
    ...
    def check_remaining(self):
        ...
        for remaining_ball in self.balls:
            if (
                remaining_ball.number != config.WHITE_BALL_NUMBER
                and remaining_ball.number != config.EIGHT_BALL_NUMBER
            ):
                stripes_remaining = (
                    stripes_remaining
        ...
        # decides if one of the players (or both) should be potting 8ball
        self.potting_8ball = {
    ...
    def first_collision(self, ball_combination):
        ...
        self.white_ball_1st_hit_8ball = (
            ball_combination[0].number == config.EIGHT_BALL_NUMBER
            or ball_combination[1].number == config.EIGHT_BALL_NUMBER
        )

        if ball_combination[0].number == config.WHITE_BALL_NUMBER:
    ...
    def potted_ball_rules(self):
        ...
        potted_stripe_count = len(
            [
                x for x in self.potted
                if x > config.EIGHT_BALL_NUMBER
            ]
        )

        potted_solid_count = len(
            [
                x for x in self.potted
                if x < config.EIGHT_BALL_NUMBER
            ]
        ...
        only_stripes_potted = (
            potted_solid_count == 0
            and potted_stripe_count > 0
        )

        only_solids_potted = (
            potted_stripe_count == 0
            and potted_solid_count > 0
        )

        if not (only_solids_potted or only_stripes_potted):
            self.turn_over(False)
            return

        selected_ball_type = (
            BallType.Striped
            if only_stripes_potted
            else BallType.Solid
        )

        if self.ball_assignment is None:
            self._assign_ball_types(selected_ball_type)

        elif (
            self.ball_assignment[self.current_player]
            != selected_ball_type
        ):
            self.turn_over(False)

    def _assign_ball_types(self, selected_ball_type):
    ...
    def first_hit_rule(self):
        ...
        # for example if the current player hits a striped ball with the white ball
        ...
        if not self.white_ball_1st_hit_is_set:
            ...
            # checks if the 8ball was the first ball hit,
            # and if so checks if the player needs to pot the 8ball
            # and if not he gets penalised
            elif self.white_ball_1st_hit_8ball:
                self.turn_over(
                    not self.potting_8ball[self.current_player]
                )